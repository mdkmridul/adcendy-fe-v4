...
| `wizard.step5.paidMediaBudgetRange` | 5 | Paid media budget range | DB-backed dropdown: the monthly-spend bands plus Not sure | Mandatory, unknown allowed | Always. Never filled from monthly spend. A draft holding pre-dropdown free text loads unselected so the client picks a band | `step5.paidMediaBudgetRange → business_constraints.paid_media_budget_range` |
...
| `wizard.step5.marketingHoursPerWeek` | 5 | How many hours a week can you or your team give marketing? | DB-backed dropdown | Mandatory in the form; optional on the backend so older drafts still commit | Always; Not sure on offer | `step5.marketingHoursPerWeek → marketing_operating_context.marketing_hours_per_week` |
| `wizard.step5.creativeCapabilities[]` | 5 | What can your team make in-house? | DB-backed multi-select | Mandatory in the form (at least one); optional on the backend | "None of these" clears the others and any other choice clears it | `step5.creativeCapabilities → marketing_operating_context.creative_capabilities` |
| `wizard.step5.deliveryDeadline` | 5 | When do you need to see results from this plan? | DB-backed dropdown | Mandatory in the form; optional on the backend | Always; Not sure on offer | `step5.deliveryDeadline → business_constraints.delivery_deadline` |
...
| `wizard.step6.dealValueBand` | 6 | Average deal or order value band | Dropdown | Mandatory, no opt-out | Blocks the step 6 save and commit when empty | `step6.dealValueBand` |
| `wizard.step6.grossMarginBand` | 6 | Gross margin band | Dropdown | Mandatory, no opt-out | Blocks the step 6 save and commit when empty | `step6.grossMarginBand` |
| `wizard.step6.closeRateBand` | 6 | Close rate band | Dropdown | Optional, default `not_tracked` | `not_tracked` is swept as a sensitivity axis rather than assumed | `step6.closeRateBand` |
| `wizard.step6.paybackWindow` | 6 | How long can you wait to earn back what it costs to win a customer? | DB-backed dropdown | Mandatory in the form; optional on the backend so older drafts still commit | Always; Not sure on offer | `step6.paybackWindow → unit_economics.payback_window` |
