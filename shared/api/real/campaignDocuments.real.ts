...
function toUploadError(status: number, requestId: string, payload: ErrorEnvelope | null): ApiError {
  const error = normalizeError(payload?.message ?? 'Document upload failed.', status, payload?.requestId ?? requestId);
...
function uploadDocument(
  ...
  return new Promise((resolve, reject) => {
    ...
    request.open('POST', `/v1/campaigns/${encodePathSegment(campaignId)}/documents`);
    ...
    request.addEventListener('load', () => {
      ...
      reject(toUploadError(request.status, requestId, payload as ErrorEnvelope | null));
    });
...
export const campaignDocumentsRealAdapter = {
  async listDocuments(campaignId: string, page = 1, pageSize = 100): Promise<CampaignDocumentList> {
  ...
  async getDownload(campaignId: string, documentId: string): Promise<CampaignDocumentDownload> {
    const response = await http<components['schemas']['DocumentDownloadEnvelope']>(
