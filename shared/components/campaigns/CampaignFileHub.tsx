...
const PRIORITY_PATTERNS = [/launch brief/i, /tracking/i, /baseline/i, /setup guide/i, /onboarding/i];
...
function FileMetricCard({ label, value, description }: { label: string; value: string; description: string }) {
  return (
                ...
                <div className="min-w-0 flex-1 space-y-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 space-y-1">
                          <h3 className="font-space-grotesk text-lg font-semibold text-foreground">{document.title}</h3>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    <div className="rounded-lg border border-border/80 bg-background/40 p-3">
                      ...
                      <p className="mt-1 break-all text-sm font-medium text-foreground">{document.fileName}</p>
                    </div>
                    <div className="rounded-lg border border-border/80 bg-background/40 p-3">
                      <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">File size</p>
                    ...
                    <div className="rounded-lg border border-border/80 bg-background/40 p-3">
                      ...
                      <p className="mt-1 text-sm font-medium text-foreground">{formatDate(document.createdAt)}</p>
                    </div>
                    <div className="rounded-lg border border-border/80 bg-background/40 p-3">
...
export function CampaignFileHub() {
  ...
  const priorityDocuments = useMemo(() => documents.filter((document) => isPriorityDocument(document)), [documents]);

  const standardDocuments = useMemo(() => documents.filter((document) => !isPriorityDocument(document)), [documents]);
  ...
  const availableCount = documents.filter((document) => getAvailabilityState(document) === 'available').length;
  const scheduledCount = documents.filter((document) => getAvailabilityState(document) === 'scheduled').length;
    ...
    return (
      <div className="space-y-6">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">Files unlock after the campaign finishes the active review stage.</p>
        </div>
        ...
        <Alert>
          ...
          <AlertDescription className="space-y-3">
            <p>
              This campaign is still being generated or reviewed, so the files section is hidden until that process is
              complete.
            </p>
  ...
  if (error) {
    ...
    const message = loadError instanceof Error ? loadError.message : 'Unable to load campaign files.';
        ...
        <Card className="border-dashed border-border bg-card">
          <Empty className="min-h-[360px] border-0">
            ...
            <EmptyHeader>
              ...
              <EmptyDescription>
                Files will appear here once the campaign team uploads onboarding materials or supporting documents.
              </EmptyDescription>
            </EmptyHeader>
            <EmptyContent className="text-muted-foreground">
              Signed links are generated only when a file is ready, so this hub stays current without exposing direct
              storage URLs.
            </EmptyContent>
          </Empty>
        </Card>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <FileMetricCard
          ...
          description={`${documents.length} file${documents.length === 1 ? '' : 's'} in the campaign document library.`}
        />
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.7fr)_320px]">
        ...
        <div className="space-y-6">
          <Card className="border-border bg-card p-5">
            <div className="space-y-4">
              <div>
                <h2 className="font-space-grotesk text-xl font-semibold text-foreground">Recent uploads</h2>
                <p className="mt-1 text-sm text-muted-foreground">The newest files added to this campaign.</p>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">{formatRelativeDate(document.createdAt)}</p>
                      </div>
                      {isPending ? (
