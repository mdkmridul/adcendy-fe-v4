...
import type { CampaignDocumentUploadInput, CampaignDocumentUploadOptions } from '@/shared/types/campaignDocument';
...
export function useCampaignDocumentUpload(campaignId: string | null) {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      return campaignDocumentsRepository.uploadDocument(campaignId, input, options);
    },
    onSuccess: async () => {
      if (campaignId) {
